...
namespace LinkDev.CRM.Library.DAL
{
    ...
    public class CRMAccessLayer
    {
        ...
        //private Libraries.Common.CrmLog Logger;
        ...
        /// <returns></returns>
        public Guid CreateSms(EntityReference regardingId, string mobileNumber, string message, Language lang = Language.Arabic)
        {
        ...
        /// <param name="lang"></param>
        /// <returns></returns>
        public Guid CreatePortalNotification(Guid userid, EntityReference regardingId, string message, Language lang = Language.Arabic)
        {
            ...
            try
            {
                ...
                PortalNotification.Attributes.Add("description", message);
                ...
                int portalLanguage = (int)(Language.Arabic); //Arabic

                if (lang == Language.English)
                {
                    portalLanguage = (int)Language.English;
                }
                PortalNotification.Attributes.Add("ldv_language", new OptionSetValue(portalLanguage));
        ...
        /// <returns></returns>
        /*  public List<OptionSetItem> RetrieveDropdownValues(string entityName, string attributeName, int langCode)
          {
              List<OptionSetItem> dropdownList = new List<OptionSetItem>();
              try
              {
                  ...
                  RetrieveAttributeResponse resp = (RetrieveAttributeResponse)cachedService.Execute(req);

                  TimeSpan diff = DateTime.Now - before;

                  double totalMSec = diff.TotalMilliseconds;
                  ...
                  for (int i = 0; i < objPckLstAttMetadata.OptionSet.Options.Count; i++)
                  {
                      OptionSetItem item = new OptionSetItem();

                      Microsoft.Xrm.Sdk.Label objLabel = objPckLstAttMetadata.OptionSet.Options[i].Label;
                      foreach (LocalizedLabel locLabel in objLabel.LocalizedLabels)
                      {
                          if (locLabel.LanguageCode == langCode)
                          {
                              if (langCode == 1025)
                              {
                                   item.ArabicLabel = locLabel.Label;
                              }
                              else if (langCode == 1033)
                              {
                                  item.EnglishLabel = locLabel.Label;
                              }
                          }
                      }

                      item.Value = objPckLstAttMetadata.OptionSet.Options[i].Value.Value;

                      if (objPckLstAttMetadata.RequiredLevel.Value == AttributeRequiredLevel.Recommended)
                      {
                          item.AttributeRequirementLevel = RequirementLevel.Recommended;
                      }
                      else if (objPckLstAttMetadata.RequiredLevel.Value == AttributeRequiredLevel.SystemRequired || objPckLstAttMetadata.RequiredLevel.Value == AttributeRequiredLevel.ApplicationRequired)
                      {
                          item.AttributeRequirementLevel = RequirementLevel.Required;
                      }
                      else
                      {
                          item.AttributeRequirementLevel = RequirementLevel.Optional;
                      }
          ...
          /// <summary>
          /// Retrieve drop down Labels
          /// </summary>
          /// <param name="entityName"></param>
          /// <param name="attributeName"></param>
          /// <param name="item"></param>
          /// <returns></returns>
          public OptionSetItem RetrieveDropdownLabel(string entityName, string attributeName, OptionSetItem item)
          {
              try
              {
                  RetrieveAttributeRequest req = new RetrieveAttributeRequest();
                  ...
                  for (int i = 0; i < objPckLstAttMetadata.OptionSet.Options.Count; i++)
                  {
                      if (item.Value == objPckLstAttMetadata.OptionSet.Options[i].Value.Value)
                      {
                          ...
                          foreach (LocalizedLabel locLabel in objLabel.LocalizedLabels)
                          {
                              if (locLabel.LanguageCode == 1025)
                              {
                                  item.ArabicLabel = locLabel.Label;
                              }
                              else if (locLabel.LanguageCode == 1033)
                              {
                                  item.EnglishLabel = locLabel.Label;
                              }
                          }
        ...
        /// <returns></returns>
        public bool DoesFieldExist(String entityName, String fieldName)  
        {
            try
            {
        ...
        public   Entity RetrivePrimaryEntityOfBpf(string bpfEntityLogicalName, Guid bpfEntityId )
        {
            try
            {
                ...
                foreach (var attribute in bpfEntity.Attributes)
                {
                    ...
                    else if (attribute.Key.Contains("bpf_incidentid"))
                    {
                        entityLogicalName = attribute.Key;
                        request = (EntityReference)bpfEntity.Attributes[entityLogicalName];
                        if (request?.Id != Guid.Empty && request.LogicalName != string.Empty)
                    ...
                    else if (attribute.Key.Contains("incidentid"))
                    {
                        entityLogicalName = attribute.Key;
                        request = (EntityReference)bpfEntity.Attributes[entityLogicalName];
                        if (request?.Id != Guid.Empty && request.LogicalName != string.Empty)
                    ...
                    else if (attribute.Key.Contains("bpf_msdyn_workorderid"))
                    {
                        entityLogicalName = attribute.Key;
                        request = (EntityReference)bpfEntity.Attributes[entityLogicalName];
                        if (request?.Id != Guid.Empty && request.LogicalName != string.Empty)
        ...
        public string GetMessageWithValues(string Message,  EntityReference regardingObject)
        {
            ...
            for (int i = 0; i < Message.Length; i++)
            {
                ...
                else if (currChar == '{')
                {
                    ...
                    if (dynamicValue.Contains(":"))
                    {
                        ...
                        lookupName = splitedDynamicValue[0];
                        fieldName = splitedDynamicValue[1];

                        fieldValue = GetFieldValue(lookupName, fieldName,   true, regardingObject).ToString();
                    ...
                    else
                    {
                        ...
                        fieldValue = GetFieldValue(lookupName, fieldName,   false, regardingObject).ToString();
                    }
        ...
        public string GetFieldValue(string lookupName, string fieldName,   bool isLookup, EntityReference regardingObject)
        {
            ...
            string fieldValue = string.Empty;           
                if (isLookup)
                {
                    ...
                    bool doesFieldExist = DoesFieldExist(regardingObject.LogicalName, lookupName);
                    if (doesFieldExist)
                    {
                        currEntity = RetrieveEntity(regardingObject.Id.ToString(), regardingObject.LogicalName, retrievedCols);
                        if (currEntity.Attributes.Contains(lookupName))
                        {
                            ...
                            bool doesFieldExistInLookup = DoesFieldExist(entityType, fieldName);
                            if (doesFieldExistInLookup) 
                            {
                                Guid entityId = ((EntityReference)currEntity.Attributes[lookupName]).Id;
                                var retrievedLookupCols = new string[] { fieldName };
                                Entity lookupEntity = RetrieveEntity(entityId.ToString(), entityType, retrievedLookupCols);
                ...
                {
                    bool doesFieldExist = DoesFieldExist(regardingObject.LogicalName, fieldName);
                    if (doesFieldExist)
                    {
        ...
        /// <returns></returns>
        public Guid GetEntityGuidByAttribute(string EntityLogicalName, string AttributeKey, object AttributeValue)  
        {
            try
            {
        ...
        public Guid GetGenericConfigurationContextUser(IOrganizationService service)
        {
            Guid ContextUser = Guid.Empty;

            try
            {
                var QEldv_genericconfiguration = new QueryExpression("ldv_configuration");
                QEldv_genericconfiguration.TopCount = 1;
                //QEldv_genericconfiguration.NoLock = true;
                QEldv_genericconfiguration.ColumnSet.AddColumns("ldv_impersonationadminsystemuserid");

                EntityCollection genericConfigurations = service.RetrieveMultiple(QEldv_genericconfiguration);
                if (genericConfigurations != null && genericConfigurations.Entities?.Count > 0)
                {
                    var genericConfiguration = genericConfigurations.Entities[0];
                    if (genericConfiguration.Contains("ldv_impersonationadminsystemuserid"))
                        ContextUser = genericConfiguration.GetAttributeValue<EntityReference>("ldv_impersonationadminsystemuserid").Id;
                }
            }
            catch (Exception ex)
            {
